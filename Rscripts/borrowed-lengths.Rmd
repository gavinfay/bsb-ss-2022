...
```{r}
load("../../BSB.Comm.Catch/Discards/Discards.at.length/Expanded.Discards.at.Length.RDATA")
```
...
exp.disc.at.length %>% 
  clean_names()  %>% 
  filter(region == "North", semester == 2) %>% 
  ggplot() +
  aes(x = length, y = exp_number, group = fleet, col = factor(fleet)) %>% 
  geom_line() +
  geom_vline(xintercept = 30) +
  #geom_point() +
  facet_wrap(~year, dir = "v", scales = "free_y")
...
disc_lens <- disclens |>
  ...
  summarize(cal = sum(numlen), .groups = "drop") |>
  mutate(cal = ifelse(season == 10 & ibin <= 6, 0, cal)) |>
  group_by(index, year, season) |>
  mutate(cal = round(cal/sum(cal, na.rm = TRUE), digits = 7)) |>
  ungroup() |>
...
```{r}
load('../../MRIPData/MRIPAssessmentInputs.RData') 
...
```{r}
### Load catch-at-length
load("../../MRIPData/recInputs.RData")
...
catch.length.orig <- recInputs$recAB1B2Size_yrs %>%
  filter(YEAR %in% yr.vec) %>%
  mutate(dead_N_B2 = N_B2 * rec.disc.mort,
         dead_N_AB1B2 = N_AB1 + dead_N_B2) %>%
  filter(L_CM_BIN <= 70)

unique(catch.length.orig$SEMESTER)
range(catch.length.orig$YEAR)

catch.length <- catch.length.orig%>%
  rename(LENGTH = L_CM_BIN) %>%
  group_by(YEAR, SEMESTER, REGION, LENGTH) %>%
  summarize(rec.land.N = sum(N_AB1),
            rec.disc.N = sum(dead_N_B2)
  ) %>%
  ungroup() %>%
  mutate(rec.catch.N = rec.land.N+rec.disc.N)
...
```{r}
  ...
  clean_names()  %>% 
  left_join(lenbins) %>% 
  group_by(year, semester, region, ibin) %>% 
  summarize(rec_disc_n = sum(rec_disc_n)) %>% 
  filter(region == "North", semester == 1) %>% 
  ggplot() +
  aes(x = ibin, y = rec_disc_n) %>% 
  geom_line() +
  geom_vline(xintercept = 30) +
  #geom_point() +
  facet_wrap(~year, dir = "v", scales = "free_y")  
